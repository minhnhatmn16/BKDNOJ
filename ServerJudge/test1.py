...
class MonoProcessMonitor:
    def __init__(self, MAX_MEMORY_MB, MAX_CPU_TIME, MAX_WALL_TIME, MAX_OUTPUT_SIZE, source_file, input_file=None, language=None):
...
def run_code():
    ...
    # executor = CodeExecutor(source_file=source_path, input_file=input_path, language=language)
    test = MonoProcessMonitor(100, 1, 1.2, 200, source_path, input_path, language)
    result = test.run()
